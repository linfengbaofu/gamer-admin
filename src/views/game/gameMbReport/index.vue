    ...
    <el-row :gutter="10" class="mb8">
      <!-- <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['game:gameMbReport:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['game:gameMbReport:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
      ...
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['game:gameMbReport:export']"
        >导出</el-button>
      </el-col>
...
<script>
import { listGameMbReport, getGameMbReport, delGameMbReport, addGameMbReport, updateGameMbReport } from "@/api/game/gameMbReport";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listGameMbReport(this.queryParams).then(response => {
        this.gameMbReportList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getGameMbReport(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateGameMbReport(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addGameMbReport(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除游戏会员报表编号为"' + ids + '"的数据项？').then(function() {
        return delGameMbReport(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('game/gameMbReport/export', {
        ...this.queryParams
      }, `gameMbReport_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
